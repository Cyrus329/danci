...
ok(idx.includes('version-badge">B175'),'B175 badge');
ok(sw.includes('word-memory-v70-b175'),'B175 service worker cache');
ok(st.includes('app.js?v=70b175')&&st.includes('spelling-hub.js?v=70b175'),'B175 cache bust');
ok(st.includes('b168-golden-baseline.js?v=70b175'),'golden baseline cache URL consistent');
...
ok(!hard.includes('checkInDays')&&!hard.includes('completedDays')&&!hard.includes('reviewActionDays'),'ancillary histories no longer hard block');
ok(app.includes('function b175AncillaryHistoryWarnings'),'ancillary history warning path exists');
...
const p=words.filter(w=>(w.groups||[]).includes('四级核心 Unit 10 政治法律'));
ok(p.length===105,'politics group 105');ok(p.filter(w=>/\s/.test(String(w.term||'').trim())).length===42,'politics phrases 42');
console.log('B175 full regression PASS');
